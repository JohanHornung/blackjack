...
class Simulation:
    ...
    def play(self):
        ...
        for _ in range(self.stacks): # iteration through the stacks
            ...
            while (len(self.cards) > 20): # the stack is switched when they are 15 cards left
                ...
                self.curr_player_results = np.zeros((1, self.player_count))
                # the dealers hand
                self.dealer_hand = []
                # each list is a player´s hand
                self.player_hands = [[] for _ in range(self.player_count)]
                # tracking down the players hands value
                self.curr_total = []
                # first draw
                for player, _ in enumerate(self.player_hands):
                    self.card = self.cards.pop(0) 
                    self.player_hands[player].append(self.card)
                ...
                self.curr_total.append(self.game.total(self.player_hands[player]))
                self.action = 0 # for tracking the player´s decision (1 == hit)
                # dealer checks for 21
                if set(self.dealer_hand) == self.blackjack:
                    for player in range(self.player_count):
                        # if the player does not have a blackjack as well he loses
                    ...
                    for player in range(self.player_count): # for each player
                        ...
                        if set(self.player_hands[player]) == self.blackjack:
                            ...
                            if (self.sim_type == "naive"): # naive simulation
                            ...
                            elif (self.sim_type == "random"): # random simulation
                            ...
                            # at this point another type of simulation has been ran as the nn need to exploit previous results
                                ...
                                while ((self.model.prediction(self.game.total(self.player_hands[player]), self.ace, 
                                    ...
                                    self.player_hands[player].append(self.cards.pop(0))
                                    self.action = 1
                                    self.curr_total.append(self.game.total(self.player_hands[player])) # adding player hand value                                    # check for bust
                                    if self.game.total(self.player_hands[player]) > 21:
                                        self.curr_player_results[0, player] = -1 # loss
                                        break # game over for this player
                ...
                # compare dealer hand to self.player_count hand but first check if dealer busted
                ...
                self.player_live_total.append(self.curr_total)
                self.games_played += 1
                # every 10.000th games played indication
    ...
    def first_dealer_card_impact(self, xlabel: str, ylabel: str, save_to="images"):
        ...
        plt.tight_layout()
        # saving
        plt.savefig(fname=f'{save_to}/{self.sim_type}_dealer_card_impact', dpi=200)
    """
    Method which creates a barplot with the probability of winning (or tie) for all the 
    ...
    def player_value_impact(self, x_label: str, y_label: str, save_to="images"):
        ...
        plt.tight_layout()
        plt.savefig(fname=f'{save_to}/{self.sim_type}_player_value_impact', dpi=200)
    ...
    def heatmap(self, x_label: str, y_label: str, save_to="images"):
        ...
        plt.savefig(fname=f'{save_to}/{self.sim_type}_heat_map', dpi=200)
    ...
    def model_comparison(self, models:list, total_stats, save_to="images"):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(fname=f'{save_to}/{self.sim_type}_player_hand_comparison', dpi=200)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(fname=f'{save_to}/{self.sim_type}_dealer_card_comparison', dpi=200)
...
#         self.axis.bar(x=(i), height=percentage, color=self.color_pick, width=0.4, label=f"{self.outcomes[i]}")
    ...
    def export_headers(self, save_to="JSON"):
        with open(f"{save_to}/{self.sim_type}_headers.json", "w", encoding="utf-8") as headers:
    ...
    def train(self):
        print("training...")
        # creating a feature list for training process
    ...
    def reset(self):
        ...
        assert self.sim_type == "smart", "The neural-net has to play!"
        # counter
        self.games_played = 0
        self.dealer_card_result = []
        self.player_card_result = []
